...
Compare the resolved types/endpoints in the source tree to the artifacts at `.bender/artifacts/plan/`:
- **Data model drift**: a struct/type/schema changed in code without updating `.bender/artifacts/plan/data-model-<ts>.md` → `medium`-severity finding.
- **API contract drift**: a new exposed endpoint / CLI flag / GraphQL field that doesn't appear in `.bender/artifacts/plan/api-contract-<ts>.md` → `high`-severity finding.

## Operating principles

- Read-only. Your write scope is empty for the source tree (only `.bender/artifacts/plan/**` is allowed, and you should not write there during `/ghu`).
