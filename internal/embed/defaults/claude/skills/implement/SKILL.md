...
7. **Run the same execution graph as `/ghu`**, pruned to the single task and its direct review/lint follow-ups. The `review-sweep` group (reviewer ∥ sentinel ∥ benchmarker — PARALLEL per the Parallel dispatch protocol in `.claude/skills/ghu/SKILL.md`) and the `docs-sweep` group (scribe — serial, after review-sweep) still run at the end unless skipped. Every `ordered: false` group MUST be dispatched as a single concurrent Agent-tool batch; never one-per-turn.
